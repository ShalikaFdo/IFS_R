...
ewAr[25] = ", TRUE); // Code page"+"\r\n"+""+"\r\n"+"define(\"EWR_CHARSET\", \"";
ewAr[26] = "\", TRUE); // Project charset"+"\r\n"+""+"\r\n"+"define(\"EWR_DBMSNAME\", '";
ewAr[27] = "', TRUE); // DBMS Name"+"\r\n"+"define(\"EWR_IS_MSACCESS\", ";
ewAr[28] = ", TRUE); // Access"+"\r\n"+"define(\"EWR_IS_MSSQL\", ";
ewAr[29] = ", TRUE); // SQL Server"+"\r\n"+"define(\"EWR_IS_MYSQL\", ";
ewAr[30] = ", TRUE); // MySQL"+"\r\n"+"define(\"EWR_IS_POSTGRESQL\", ";
ewAr[31] = ", TRUE); // PostgreSQL"+"\r\n"+"define(\"EWR_IS_ORACLE\", ";
...
ewAr[36] = ", TRUE); // Check post token"+"\r\n"+""+"\r\n"+"// Session timeout time"+"\r\n"+"define(\"EWR_SESSION_TIMEOUT\", ";
ewAr[37] = ", TRUE); // Session timeout time (minutes)"+"\r\n"+""+"\r\n"+"// Session keep alive interval"+"\r\n"+"define(\"EWR_SESSION_KEEP_ALIVE_INTERVAL\", ";
ewAr[38] = ", TRUE); // Session keep alive interval (seconds)"+"\r\n"+"define(\"EWR_SESSION_TIMEOUT_COUNTDOWN\", ";
ewAr[39] = ", TRUE); // Session timeout count down interval (seconds)"+"\r\n"+""+"\r\n"+"// General"+"\r\n"+"$EWR_ROOT_RELATIVE_PATH = \"";
ewAr[40] = "\"; // Relative path of app root"+"\r\n"+"define(\"EWR_DEFAULT_DATE_FORMAT\", \"";
ewAr[41] = "\", TRUE); // Default date format"+"\r\n"+"define(\"EWR_DATE_SEPARATOR\", \"";
ewAr[42] = "\", TRUE); // Date separator"+"\r\n"+"define(\"EWR_UNFORMAT_YEAR\", 50, TRUE); // Unformat year"+"\r\n"+"define(\"EWR_RANDOM_KEY\", '";
ewAr[43] = "', TRUE); // Random key for encryption"+"\r\n"+"define(\"EWR_PROJECT_STYLESHEET_FILENAME\", \"";
ewAr[44] = "\", TRUE); // Project stylesheet file name"+"\r\n"+"define(\"EWR_CHART_WIDTH\", 550, TRUE);"+"\r\n"+"define(\"EWR_CHART_HEIGHT\", 440, TRUE);"+"\r\n"+"define(\"EWR_CHART_SHOW_BLANK_SERIES\", ";
ewAr[45] = ", TRUE); // Show blank series"+"\r\n"+"define(\"EWR_CHART_SHOW_ZERO_IN_STACK_CHART\", ";
ewAr[46] = ", TRUE); // Show zero in stack chart"+"\r\n"+""+"\r\n"+"// Drill down setting"+"\r\n"+"define(\"EWR_USE_DRILLDOWN_PANEL\", ";
ewAr[47] = ", TRUE); // Use popup panel for drill down"+"\r\n"+""+"\r\n"+"";
...
ewAr[140] = "\", TRUE); // Export PDF CSS styles"+"\r\n"+"define(\"EWR_PDF_MEMORY_LIMIT\", \"";
ewAr[141] = "\", TRUE); // Memory limit"+"\r\n"+"define(\"EWR_PDF_TIME_LIMIT\", ";
ewAr[142] = ", TRUE); // Time limit"+"\r\n"+"?>"+"\r\n"+"";
...
ewSB.Append(ewAr[3]);
	...
	var sUploadPath = PROJ.UploadPath.replace("\\", "/");
	if (sUploadPath.substr(0,1) == "/") sUploadPath = sUploadPath.substr(1); // Remove leading "/"
	if (sUploadPath.substr(sUploadPath.length-1) != "/") sUploadPath += "/"; // Add trailing "/"
...
ewSB.Append(ewAr[81]);
ewSB.Append(ew_Quote2(ew_QuotedName(DB.UserLevelIdFld)));
ewSB.Append(ewAr[82]);
ewSB.Append(ew_Quote2(ew_QuotedName(DB.UserLevelNameFld)));
ewSB.Append(ewAr[83]);
...
ewSB.Append(ewAr[86]);
ewSB.Append(ew_Quote2(ew_QuotedName(DB.UserLevelPrivTblNameFld)));
ewSB.Append(ewAr[87]);
ewSB.Append(ew_Quote2(DB.UserLevelPrivTblNameFld));
ewSB.Append(ewAr[88]);
ewSB.Append(ew_Quote2(ew_QuotedName(DB.UserLevelPrivUserLevelFld)));
ewSB.Append(ewAr[89]);
ewSB.Append(ew_Quote2(ew_QuotedName(DB.UserLevelPrivPrivFld)));
ewSB.Append(ewAr[90]);
 } 
ewSB.Append(ewAr[91]);
...
ewSB.Append(ewAr[108]);
	...
	var sEmailTableDBID = "DB";
	if (DB.Tables.TableExist(sEmailLogTable)) {
		var EMAILTABLE = DB.Tables(sEmailLogTable);
		ew_SetDb(EMAILTABLE.TblName);
		var sEmailTableDBID = gsDbId;
	}

ewSB.Append(ewAr[109]);
ewSB.Append(ew_Quote(sEmailTableDBID));
...
ewSB.Append(ew_Val(PROJ.GetV("UseCustomTemplate")));
