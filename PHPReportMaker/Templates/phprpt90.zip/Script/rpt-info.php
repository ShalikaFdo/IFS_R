...
##-->
	...
	function __construct() {
		...
		$this->TableName = '<!--##=ew_SQuote(gsTblName)##-->';
		$this->TableType = '<!--##=TABLE.TblType##-->';
		$this->DBID = '<!--##=ew_SQuote(sTableDbId)##-->';
...
##-->
		// <!--##=gsChartName##-->
		$this-><!--##=gsChartVar##--> = new crChart($this->DBID, '<!--##=gsTblVar##-->', '<!--##=ew_SQuote(gsTblName)##-->', '<!--##=gsChartVar##-->', '<!--##=ew_SQuote(gsChartName)##-->', '<!--##=ew_SQuote(sChartXFldName)##-->', '<!--##=ew_SQuote(sChartYFldName)##-->', '<!--##=ew_SQuote(sChartSFldName)##-->', <!--##=iChartType##-->, '<!--##=sChartSummaryType##-->', <!--##=iChartWidth##-->, <!--##=iChartHeight##-->);
...
*/
	...
	##-->
		$this->SummaryFields[<!--##=i##-->] = new crSummaryField('<!--##=ew_SQuote(sFldVar)##-->', '<!--##=ew_SQuote(sFldName)##-->', '<!--##=ew_SQuote(sFld)##-->', '<!--##=ew_SQuote(sFldSummaryType)##-->');
		$this->SummaryFields[<!--##=i##-->]->SummaryCaption = <!--##=arSmryFlds[i]['FldSummaryCaption']##-->;
		$this->SummaryFields[<!--##=i##-->]->SummaryVal = &ewr_InitArray($this->ColCount+1, NULL);
		$this->SummaryFields[<!--##=i##-->]->SummaryValCnt = &ewr_InitArray($this->ColCount+1, NULL);
		$this->SummaryFields[<!--##=i##-->]->SummaryCnt = &ewr_Init2DArray($this->ColCount+1, $nGrps+1, NULL);
		$this->SummaryFields[<!--##=i##-->]->SummarySmry = &ewr_Init2DArray($this->ColCount+1, $nGrps+1, NULL);
		$this->SummaryFields[<!--##=i##-->]->SummarySmryCnt = &ewr_Init2DArray($this->ColCount+1, $nGrps+1, NULL);
		$this->SummaryFields[<!--##=i##-->]->SummaryInitValue = <!--##=arSmryFlds[i]['FldSummaryInitValue']##-->;
	<!--##
			}
		};
	##-->
	...
	<!--## } else { ##-->
		...
		for ($is = 0; $is < $cnt; $is++) {
			$smry = &$this->SummaryFields[$is];
			for ($colcnt = 1; $colcnt <= $this->ColCount; $colcnt++) {
				$sFld = ewr_CrossTabField($smry->SummaryType, $smry->FldExpression, $this->getColumnField(), $this->getColumnDateType(), $this->Col[$colcnt]->Value, "<!--##=sColFldQc##-->", "C" . $is . $colcnt, $this->DBID);
				if ($sSqlFlds <> "")
					$sSqlFlds .= ", ";
				$sSqlFlds .= $sFld;
			}
...
##-->
	...
	<!--## if (bParentUserID) { ##-->
			$sSql = "SELECT <!--##=ew_Quote2(sUserIDFldName)##--> FROM <!--##=ew_Quote2(sFromPart)##--> WHERE <!--##=ew_Quote2(sParentUserIDFldName)##--> = " . ewr_QuotedValue($sUserID, <!--##=sUserIDFldTypeName##-->, $this->DBID) . " OR <!--##=ew_Quote2(sUserIDFldName)##--> = " . ewr_QuotedValue($sUserID, <!--##=sUserIDFldTypeName##-->, $this->DBID);
	<!--## } else { ##-->
			$sSql = "SELECT <!--##=ew_Quote2(sUserIDFldName)##--> FROM <!--##=ew_Quote2(sFromPart)##--> WHERE <!--##=ew_Quote2(sUserIDFldName)##--> = " . ewr_QuotedValue($sUserID, <!--##=sUserIDFldTypeName##-->, $this->DBID);
	<!--## } ##-->
		}
	...
	<!--## if (bParentUserID) { ##-->
		...
		if (EWR_USER_ID_IS_HIERARCHICAL) {
			...
			while ($sUserIDList <> $sCurUserIDList) {
				$arUserWrk = array();
				$sSql = "SELECT <!--##=ew_Quote2(sUserIDFldName)##--> FROM <!--##=ew_Quote2(sFromPart)##--> WHERE <!--##=ew_Quote2(sParentUserIDFldName)##--> IN (" . $sCurUserIDList . ") OR <!--##=ew_Quote2(sUserIDFldName)##--> = " . ewr_QuotedValue($sUserID, <!--##=sUserIDFldTypeName##-->, $this->DBID);
	...
	function UserIDList($ar) {
		...
		if (is_array($ar)) {
			...
			for ($i = 0; $i < $cntar; $i++) {
				if ($sWrk <> "")
					$sWrk .= ", ";
				$sWrk .= ewr_QuotedValue($ar[$i], <!--##=sUserIDFldTypeName##-->, $this->DBID);
			}
		}
		return $sWrk;
	}
