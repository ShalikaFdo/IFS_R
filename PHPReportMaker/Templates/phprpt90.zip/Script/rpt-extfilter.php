...
if (is_array($<!--##=sColDateFldObj##-->->ValueList)) {
	...
	for ($yearIdx = 0; $yearIdx < $cntyr; $yearIdx++) {
		...
		$yearSelected = (strval($yearValue) == strval($<!--##=sColDateFldObj##-->->SelectionList)) ? " selected" : "";
?>
	...
	for (var i = 0; i < nFldCount; i++) {
		if (GetFldObj(arFlds[i])) {
			...
			if (IsExtendedFilter(goFld)) {
				if (!IsTextFilter(goFld)) {
##-->
			$this->SetSessionDropDownValue($<!--##=sFldObj##-->->DropDownValue, $<!--##=sFldObj##-->->SearchOperator, '<!--##=gsFldParm##-->'); // Field <!--##=gsFldName##-->
<!--##
				} else {
##-->
...
##-->
			// Field <!--##=gsFldName##-->
			if ($this->GetDropDownValue($<!--##=sFldObj##-->)) {
				$bSetupFilter = TRUE;
...
##-->
			$sWrk = "";
			$this->BuildDropDownFilter($<!--##=sFldObj##-->, $sWrk, $<!--##=sFldObj##-->->SearchOperator);
			ewr_LoadSelectionFromFilter($<!--##=sFldObj##-->, $sWrk, $<!--##=sFldObj##-->->SelectionList, $<!--##=sFldObj##-->->DropDownValue);
<!--##
					}
				} else {
...
##-->
	...
	function GetDropDownValue(&$fld) {
		...
		if (isset($_GET["<!--##=pfxDdVal##-->$parm"])) {
			$fld->DropDownValue = ewr_StripSlashes(@$_GET["<!--##=pfxDdVal##-->$parm"]);
			return TRUE;
		}
		return FALSE;
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		...
		$_SESSION['<!--##=pfxSo1##--><!--##=gsTblVar##-->_' . $parm] = $so;
	}
...
##-->
		...
		if (is_array($this-><!--##=sColDateFldParm##-->->SelectionList)) {
			$sWrk = ewr_JoinArray($this-><!--##=sColDateFldParm##-->->SelectionList, ", ", EWR_DATATYPE_NUMBER, 0, $this->DBID);
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $ReportLanguage->Phrase("Year") . "</span>";
...
##-->
		if (is_array($<!--##=sFldObj##-->->SelectionList))
			$sWrk = ewr_JoinArray($<!--##=sFldObj##-->->SelectionList, ", ", <!--##=GetFieldTypeName(goFld.FldType)##-->, 0, $this->DBID);
<!--##
				}
##-->
		$sFilter = "";
...
##-->
		if (array_key_exists("sv_<!--##=gsFldParm##-->", $filter)) {
			...
			if (strpos($sWrk, "||") !== FALSE)
				$sWrk = explode("||", $sWrk);
			$this->SetSessionDropDownValue($sWrk, @$filter["sv_<!--##=gsFldParm##-->"], "<!--##=gsFldParm##-->");
			$bRestoreFilter = TRUE;
		}
<!--##
				} else if (IsExtendedFilter(goFld) && IsTextFilter(goFld)) {
...
##-->
				$sFilter = ewr_FilterSQL($<!--##=sFldObj##-->, "<!--##=ew_Quote(gsFld)##-->", EWR_DATATYPE_NUMBER, $this->DBID);
<!--## } else { ##-->
				$sFilter = ewr_FilterSQL($<!--##=sFldObj##-->, "<!--##=ew_Quote(gsFld)##-->", <!--##=GetFieldTypeName(goFld.FldType)##-->, $this->DBID);
<!--## } ##-->
	<!--## if (SYSTEMFUNCTIONS.ServerScriptExist("Table","Page_Filtering")) { ##-->
				// Call Page Filtering event
...
##-->
		// Year Filter
		if (is_array($this-><!--##=sColDateFldParm##-->->SelectionList)) {
			ewr_AddFilter($sWrk, ewr_FilterSQL($this-><!--##=sColDateFldParm##-->, "<!--##=ew_Quote(sColDateFld)##-->", EWR_DATATYPE_NUMBER, $this->DBID));
		}
<!--##
    }
##--> 
...
##-->
			case "<!--##=sColFldParm##-->":
				$sql = "<!--##=ew_DbGrpSql("y",0)##-->";
				$sql = str_replace("%s", "@" . $target, $sql) . " = " . ewr_QuotedValue($this-><!--##=sColDateFldParm##-->->SelectionList, EWR_DATATYPE_NUMBER, $this->DBID);
				$colsql = "";
...
##-->
			case "<!--##=sColFldParm##-->":
				if ($rowtype == 0) { // Add year filter for detail record
					$sql = "<!--##=ew_DbGrpSql("y",0)##-->";
					$sql = str_replace("%s", "@" . $target, $sql) . " = " . ewr_QuotedValue($this-><!--##=sColDateFldParm##-->->CurrentValue, EWR_DATATYPE_NUMBER, $this->DBID);
				} elseif (is_array($this-><!--##=sColDateFldParm##-->->SelectionList)) { // Year popup filter
					$sql = "<!--##=ew_DbGrpSql("y",0)##-->";
					$sql = str_replace("%s", "@" . $target, $sql) . " IN (" . ewr_JoinArray($this-><!--##=sColDateFldParm##-->->SelectionList, ", ", EWR_DATATYPE_NUMBER, $this->DBID) . ")";
				}
				$colsql = "";
				if ($parm >= 1 && $parm <= $this->ColCount) {
					$colsql = "<!--##=ew_DbGrpSql(sqltype,0)##-->";
...
##-->
			case "<!--##=gsFldParm##-->":
				if ($parm >= 1 && $parm <= $this->ColCount) {
					$sql = "@" . $target . " = " . ewr_QuotedValue($this->Col[$parm]->Value, <!--##=GetFieldTypeName(sColFldType)##-->, $this->DBID);
				}
				break;
<!--##
					} // End column field
