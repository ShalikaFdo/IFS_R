...
<td data-field="<!--##=sFldParm##-->">
	<div<!--##=sTDStyle##-->><span class="ewTableHeaderCaption"><?php echo $<!--##=sSrcFldObj##-->->FldCaption() ?> (<?php echo $ReportLanguage->Phrase("Rpt<!--##=sSummaryType##-->") ?>)</span></div>
</td>
<?php } ?>
	<!--##
				}
...
##-->
<?php if (<!--##=sFldCount##--> > 0) { ?>
		<td colspan="<?php echo (<!--##=sFldCount##-->) ?>"<?php echo $<!--##=sFldObj##-->->CellAttributes() ?>><?php echo str_replace(array("%v", "%c"), array(<!--##=sSmryPrefix##-->$<!--##=sFldObj##-->->GroupViewValue<!--##=sSmrySuffix##-->, $<!--##=sFldObj##-->->FldCaption()), $ReportLanguage->Phrase("RptSumHead")) ?> <span class="ewDirLtr">(<?php echo ewr_FormatNumber($<!--##=gsPageObj##-->->Cnt[<!--##=lvl##-->][0],0,-2,-2,-2) ?><!--##@RptDtlRec##-->)</span></td>
<?php } ?>
</tr>
<!--##
...
<!--##
				...
				sGrpFldObj = arGrpFlds[i]['FldObj'];
##-->
		<td colspan="<?php echo ($<!--##=gsPageObj##-->->GrpFldCount - <!--##=i##-->) ?>"<?php echo $<!--##=sGrpFldObj##-->->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("Rpt<!--##=arSmry[0][j]##-->") ?></td>
<!--##
				for (var k = 1; k <= nDtls; k++) {
					sFldName = arDtlFlds[k-1]['FldName'];
...
<?php if ($<!--##=gsPageObj##-->->GrpFldCount > 0) { ?>
		<td colspan="<?php echo $<!--##=gsPageObj##-->->GrpFldCount ?>" class="ewRptGrpAggregate"><?php echo $ReportLanguage->Phrase("Rpt<!--##=arSmry[0][j]##-->") ?></td>
<?php } ?>
	<!--##
		}
	##-->
...
<!--## } ##-->
	...
	##-->
			if (is_array($<!--##=sFldObj##-->->DbValue) || is_object($<!--##=sFldObj##-->->DbValue)) // Byte array
				$<!--##=sFldObj##-->->DbValue = ewr_BytesToStr($<!--##=sFldObj##-->->DbValue);
	<!--##
				}
			}
		}
