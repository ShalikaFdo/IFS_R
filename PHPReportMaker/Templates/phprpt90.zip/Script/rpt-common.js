...
function GetFieldValues(f) {
	...
	var list = "";
	var val = "";
	if (SYSTEMFUNCTIONS.IsBoolFld(f) && ew_GetFieldHtmlTag(f) == "CHECKBOX") {
		var ar = values.split("\r\n");
...
// Check if Boolean Checkbox filter
function IsBooleanCheckboxFilter(f) {
	return f.FldHtmlTag == "CHECKBOX" && SYSTEMFUNCTIONS.IsBoolFld(f);
}
...
function IsBlobView() {
	return FIELD && FIELD.FldHtmlTag == "FILE" && FIELD.FldList && (FIELD.FldType == 205 || FIELD.FldType == 204 || FIELD.FldType == 128);
}
...
function SqlTableName(t) {
	...
	if (t.TblType == "LINKTABLE")
		name = ew_QuotedName(t.LinkTableName);
	else
