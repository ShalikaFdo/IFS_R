...
function ewr_SetSessionTimer() {
	if (typeof EWR_IS_LOGGEDIN == "undefined" || !EWR_IS_LOGGEDIN)
		return; // Skip if not logged in
	var $ = jQuery, timeoutTime, timer1, timer2, timer3, counter, $dlg = $("#ewrTimer"),
		useKeepAlive = ((EWR_SESSION_KEEP_ALIVE_INTERVAL > 0 || EWR_IS_AUTOLOGIN) && EWR_SESSION_URL);
	// Keep alive
		...
		timer2 = $.later(1000, null, function() { // Create a timer that runs every second												
			if (counter > 0) {
				...
				$.later(1000, null, function() {
					$dlg.modal("hide");
					window.location = EWR_LOGOUT_URL + "?expired=1";
...
function ewr_ParseDate(data, format) {
	...
	if ($.isNumber(format) && format >=0 && format <= 17) {
		var f, def = EWR_DEFAULT_DATE_FORMAT.toUpperCase(), sep = EWR_DATE_SEPARATOR;
		switch (format) {
			case 0: case 2: case 8: f = def + " HH:mm:ss"; break; // EWR_DEFAULT_DATE_FORMAT + " %H:%M:%S"
...
function ewr_FormatDate(data, format) {
	return moment(data).format(format || EWR_DEFAULT_DATE_FORMAT.toUpperCase());
}
