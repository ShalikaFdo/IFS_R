...
function ewr_AutoLoad($class) {
	...
	<!--## } ##-->
		...
		$file = str_replace("%cls%", substr($class, 2) . "rpt", "<!--##=ew_Quote(fn)##-->");
	<!--## if (PROJ.OutputNameLCase) { ##-->
...
##-->
		<!--##=sCond##--> ($dbid == "<!--##=dbid##-->" || $dbid === <!--##=i##-->) // <!--##=dbid##-->
			$dbclass = "cr<!--##=sLinkDbVar##-->_db";
<!--##
	}
	if (dbcnt > 0) {
##-->
		else // DB
			$dbclass = "cr<!--##=sDbVar##-->_db";
<!--##
...
//	var $SelectedOptionTemplate = '<option value="{langid}" selected>{langdesc}</option>'; // Selected OPTION tag template
	...
	<!--##~SYSTEMFUNCTIONS.GetServerScript("Global","Language_Load")##-->
	...
	<!--## if (bUserTable) { ##-->
		...
		##-->
			...
			if ($rs = $UserTableConn->Execute($sSql)) {
				if (!$rs->EOF) {
					$ValidateUser = $CustomValidateUser || ewr_ComparePassword($rs->fields('<!--##=ew_SQuote(PROJ.SecPasswdFld)##-->'), $pwd, $encrypted);
...
<!--## } ##-->
	...
	<!--##
		if (bUserID) {
			// User id field
			FIELD = SECTABLE.Fields(DB.SecuUserIDFld);
			sFld = ew_FieldSqlNameBase(FIELD, SECTABLE);
...
<!--## } ##-->
	...
	function UserIDList() {
		...
		$len = count($ar);
		for ($i = 0; $i < $len; $i++)
			$ar[$i] =  ewr_QuotedValue($ar[$i], <!--##=sFldDataType##-->, EWR_USER_TABLE_DBID);
		return implode(", ", $ar);
	}
	...
	function ParentUserIDList($userid) {
		...
		if (trim(strval($userid)) == strval(CurrentUserID())) {
			if (strval(CurrentParentUserID()) <> "")
				$result = ewr_QuotedValue(CurrentParentUserID(), <!--##=sFldDataType##-->, EWR_USER_TABLE_DBID);
			return $result;
		}
		...
		if (!EWR_USER_ID_IS_HIERARCHICAL) {
			...
			return ewr_QuotedValue(CurrentUserID(), <!--##=sFldDataType##-->, EWR_USER_TABLE_DBID);
			...
			for ($i = 0; $i < $len; $i++) {
				if (strval($ar[$i]) <> trim(strval($userid))) {
					if ($result <> "")
						$result .= ", ";
					$result .= ewr_QuotedValue($ar[$i], <!--##=sFldDataType##-->, EWR_USER_TABLE_DBID);
				}
			}
			return $result;
...
function ewr_GetFileUploadUrl($fld, $val, $resize = FALSE, $encrypt = EWR_ENCRYPT_FILE_PATH, $urlencode = TRUE) {
	if (!ewr_EmptyStr($val)) {
		...
		if ($encrypt) {
			...
			$fn = "<!--##=sFnFileViewer##-->?t=" . ewr_Encrypt($fld->TblName, $key) ."&fn=" . ewr_Encrypt($path . $val, $key);
			if ($resize)
				$fn .= "&width=" . $fld->ImageWidth . "&height=" . $fld->ImageHeight;
		} elseif ($resize) {
			$fn = "<!--##=sFnFileViewer##-->?t=" . rawurlencode($fld->TblName) . "&fn=" . ewr_UrlEncodeFilePath($path . $val) .
