...
$sSql = ewr_BuildReportSql($sSql, $sChartFilter, $Chart->SqlGroupBySeries, "", $Chart->SqlOrderBySeries, $<!--##=gsPageObj##-->->Filter, "");
<!--## } ##-->
...
ewr_AddFilter($sChartFilter, $Table->getSqlWhere());
$sSql = ewr_BuildReportSql($sSql, $sChartFilter, $Chart->SqlGroupBy, "", $Chart->SqlOrderBy, $<!--##=gsPageObj##-->->Filter, "");
<!--## } ##-->
$Chart->ChartSql = $sSql;
...
<option value="1"<?php if ($<!--##=sChartObj##-->->ChartSortType == "1") echo " selected" ?>><?php echo $ReportLanguage->Phrase("ChartOrderXAsc") ?></option>
<option value="2"<?php if ($<!--##=sChartObj##-->->ChartSortType == "2") echo " selected" ?>><?php echo $ReportLanguage->Phrase("ChartOrderXDesc") ?></option>
<option value="3"<?php if ($<!--##=sChartObj##-->->ChartSortType == "3") echo " selected" ?>><?php echo $ReportLanguage->Phrase("ChartOrderYAsc") ?></option>
<option value="4"<?php if ($<!--##=sChartObj##-->->ChartSortType == "4") echo " selected" ?>><?php echo $ReportLanguage->Phrase("ChartOrderYDesc") ?></option>
</select>
...
if (EXT_CHART != null) {
	...
	var bUseFusionChartExportAtClient = false; // v9
	for (var enumerator = new Enumerator(EXT_CHART.Properties); !enumerator.atEnd(); enumerator.moveNext()) {
		...
		if (name != "ChartSeq" && name != "ChartName" && name != "useGridComponent" && name != "gridComponentHeight" && ew_IsNotEmpty(name) && ew_IsNotEmpty(value)) {
			if (/^grid/.test(name)) { // Grid parameters
				...
				} else if (name == "exportAtClientSide") { // v9
					bUseFusionChartExportAtClient = (String(value) == "1"); // v9
				} else {
	...
	if (bUseFusionChartExport) { // v8.1
		if (bUseFusionChartExportAtClient) { // v9
			parmdata += "array(\"exportEnabled\", \"1\"),\r\n\t" +
			 "array(\"exportAtClientSide\", \"1\")";
		} else {
			parmdata += "array(\"exportEnabled\", \"1\")";
		}
	}
