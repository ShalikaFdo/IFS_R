...
function() 
{
    $('#InteviewPanelSaveBtn').click(function(){
		...
		if(($('#interviewName').val())==""){
			swal("Oops...","Please enter interview name","error");
		}else if(($('#dur').val())==""){
			swal("Oops...","Please enter interview duration per candidate","error");
		}else if(($('#tableCommon :input').val())==""){
			swal("Oops...","Please enter interview schedule","error");
		}else{			
...
jQuery(function(){
  ...
  <div class="divDarkRectangle">
	...
	<div id = "panelTable" >
	<table width="100%" border="1" cellpadding="5" class="sortable" id="tableCommon1" >
	  <tbody>
		<tr>
  ...
  <strong class="headingsInADiv"> Convenient Time Slots</strong><br/><hr/>
    ...
    <input type="number" id="dur" class="textInputInterviewPanel" placeholder="Interview Duration per Candidate"><strong>&nbsp;minutes</strong><br/>
    </div>
    
  <input type="button" id="InteviewPanelSaveBtn" value="Save"/>
  <a href="interviews.php?rsid=<?php echo $rSessionID?>&rname=<?php echo $rName?>&rjob=<?php echo $rJob?>&rdate=<?php echo $rDate?>&rstatus=<?php echo $rStatus?>"><input type="button" id="interviewPanelCancelBtn" value="Cancel" /></a>
  <!--<a href="interviewSchedule.php?rsid=<?php //echo $rSessionID?>&rjob=<?php //echo $rJob?>"><input type="button" id="interviewSchedule" value="Schedule"/></a> -->
