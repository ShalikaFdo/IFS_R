...
<div>
  ...
  <aside class="asideLeftIcons">
	  <nav>
		<a href="candidateList.php?rsid=<?php echo $rSessionID?>&rname=<?php echo $rName?>&rjob=<?php echo $rJob?>&rdate=<?php echo $rDate?>&rstatus=<?php echo $rStatus?>" class="aBack">Back</a> 
  ...
  <tbody>
    <tr>
	  ...
	  <?php	
		...
		$data = mysqli_query($connect, "SELECT IntID, Name,EvaCriID, duration FROM interview WHERE RSID='$rSessionID'");
		while($raw = $data->fetch_row()) { ?>
		<!--<script> var id = <?php echo(json_encode($raw[0])); ?>;</script>--->
        <?php if($raw[2]==0){?>
            <tr class="tableRow" href="evaluationCriteriaAdd.php?rsid=<?php echo $rSessionID?>&int=<?php echo $raw[0]?>">
                <td class="tableData" name="intName" id = <?php echo $raw[0] ?> ><?php echo $raw[1] ?></td>
            </tr>
            <?php } else{ ?>
            <tr class="tableRow" href="interviewCandidates.php?&int=<?php echo $raw[0]?>">
                <td class="tableData" name="intName" id = <?php echo $raw[0] ?> ><?php echo $raw[1] ?></td>
            </tr>
         <?php } ?>
